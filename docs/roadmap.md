...
- **V0.2 — Agent access (current):** read-only REST API and Streamable HTTP MCP server exposing `get_market_state`, `get_funding_rate_state`, `get_open_interest_state`, `get_recent_events`, `get_event_detail`, and `get_abnormal_events` through one application query layer.
